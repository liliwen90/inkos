...
import { listAvailableGenres, readGenreProfile, getBuiltinGenresDir } from "@actalk/hintos-core";
  ...
  .action(async (id: string) => {
    try {
      ...
      try {
        ...
        logError(`Built-in genre "${id}" not found. Use 'hintos genre list' to see available genres.`);
        process.exit(1);
        return;
      }
