# HintOS 竞品调研 (2026-03-14)

## 基本信息
- **GitHub**: https://github.com/Narcooo/hintos
- **版本**: v0.3.6 (首次提交仅3天前，迭代极快)
- **技术栈**: TypeScript 单仓库, pnpm workspaces, Commander.js CLI
- **包名**: @actalk/hintos (npm)
- **许可证**: MIT 开源
...
- 检测历史：detection_history.json + hintos detect --stats

## 题材规则体系
内置5题材：玄幻、仙侠、都市、恐怖、通用
每个题材含：章节类型、禁忌清单、疲劳词、语言铁律(✗→✓示例)、审计维度子集
- 可 `hintos genre copy` 复制到项目中自定义
- 可 `hintos genre create` 从零创建新题材
...
- **写入锁**: 文件锁防并发写入
- **守护进程**: `hintos up` 后台按计划写章
- **通知推送**: Telegram/飞书/企微/Webhook (HMAC-SHA256签名)
- **外部Agent集成**: 原子命令+--json输出，可被OpenClaw等调用
- **自然语言Agent模式**: `hintos agent "写下一章"` — 内置9个tool-use工具
- **LLM支持**: OpenAI + Anthropic原生SDK + 所有兼容接口
- **诊断**: `hintos doctor` 含API连通性测试

## 三种使用模式
1. 完整管线：`hintos write next 书名 --count 5`
2. 原子命令：`hintos draft/audit/revise` 单独执行+--json
3. 自然语言Agent：`hintos agent "帮我写一本都市修仙"`
...
### 战略建议（已执行）
HintOS是MIT开源，核心质量保障体系远超我们。
✅ 已决策：Fork HintOS → 在其上构建Electron GUI → 移植我们的独有优势 = 降维打击。
✅ 已Fork: https://github.com/liliwen90/hintos
✅ 已Clone到本地: F:\011 Projects\009-HintOS
✅ 深度代码审计完成
✅ 架构方案确定: Adapter Pattern (不改core一行代码)
详细规划 → `/memories/repo/hintos-gui-plan.md`
