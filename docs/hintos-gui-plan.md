# HintOS Studio — GUI 桌面应用改造规划 (2026-03-14)

## ⚡ 战略决策（已确认）
- **目标**: 基于 HintOS (MIT) 的 `@actalk/hintos-core` 构建 Electron GUI 桌面应用
- **核心逻辑**: HintOS引擎级质量 + 我们的GUI易用性 + 独有增值功能 = **降维打击**
- **Fork仓库**: `https://github.com/liliwen90/hintos`
- **本地路径**: `F:\011 Projects\009-HintOS`
- **原工具路径**: `F:\011 Projects\008 Ai-Novelist` (v1.5.1, 继续保留作参考)

## 🔴 关键架构决策：Adapter Pattern（不改Core一行代码）

### 为什么不改Core
HintOS迭代极快（3天到v0.3.6），如果我们改了core代码，每次合并上游都会冲突。
**正确做法**：通过Adapter层包装core API，零侵入。

### 三层架构
```
上游 HintOS Core (@actalk/hintos-core) ← 不改一行，npm依赖或git submodule
        ↓
...
### Git 双remote管理
```bash
git remote add upstream https://github.com/Narcooo/hintos.git
# 日常更新：
git fetch upstream
...
{
  "dependencies": {
    "@actalk/hintos-core": "0.3.6"  // 锁定版本，不用 ^
  }
}
```
升级流程：`npm update @actalk/hintos-core` → 跑适配器测试 → 修适配器 → 发版
...
| CLI来源 | 搬到Studio | 核心逻辑 |
|---------|-----------|---------|
| init.ts | project-init.ts (~60行) | 创建hintos.json+.env+目录 |
| config.ts | config-manager.ts (~100行) | 全局~/.hintos/.env + 项目.env + hintos.json加载合并 |
...
| CLI命令 | GUI页面 | 优先级 |
|---------|---------|--------|
| hintos book create | 创建书籍表单 | P1 |
| hintos write next [--count N] | 写作控制台+进度 | P1 |
| hintos status | 仪表盘 | P1 |
| hintos review list/approve | 章节管理+审阅 | P1 |
| hintos export | 导出按钮 | P1 |
| hintos config set/set-global | LLM配置页 | P1 |
| hintos audit/revise | 写作控制台内（自动或手动） | P1 |
| hintos detect | AIGC检测面板 | P2 |
| hintos style analyze/import | 文风分析面板 | P2 |
| hintos genre list/show/copy/create | 题材编辑器 | P2 |
| hintos book update | 书籍设置面板 | P2 |
| hintos radar scan | 雷达面板 | P3 |
| hintos agent | 自然语言对话界面 | P3 |
| hintos up/down | 守护进程控制 | P3 |
| hintos doctor | 诊断页面 | P3 |
| hintos analytics | 统计面板 | P3 |
| hintos write rewrite | 章节重写（带回滚） | P2 |
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 前端框架 | **React + TypeScript** | HintOS也是TS, 类型共享 |
...
## 当前状态 (2026-03-16)
- ✅ fork完成: https://github.com/liliwen90/hintos
- ✅ clone到本地: F:\011 Projects\009-HintOS
...
- ✅ **Core API 兼容性审计** (2026-03-15) — commit 4008b94 (+145行 / -39行)
  ...
  - **upstream remote 已添加**: `git remote add upstream https://github.com/Narcooo/hintos.git`
