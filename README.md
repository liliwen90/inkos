...
| AIGC 检测 API | 外部 API 集成（GPTZero / Originality / 自定义端点），`hintos detect` 命令 |
| 文风指纹学习 | 从参考文本提取 StyleProfile（句长、TTR、修辞特征），注入 Writer prompt |
| 反检测改写 | ReviserAgent `anti-detect` 模式，检测→改写→重检测循环 |
| 检测反馈闭环 | `detection_history.json` 记录每次检测/改写结果，`hintos detect --stats` 查看统计 |

```bash
hintos style analyze reference.txt         # 分析参考文本文风
hintos style import reference.txt 吞天魔帝  # 导入文风到书
hintos detect 吞天魔帝 --all               # 全书 AIGC 检测
hintos detect --stats                      # 检测统计
```
...
```bash
hintos book create --title "吞天魔帝" --genre xuanhuan
```

题材规则可以查看、复制到项目中修改、或从零创建：

```bash
hintos genre list                      # 查看所有题材
hintos genre show xuanhuan             # 查看玄幻的完整规则
hintos genre copy xuanhuan             # 复制到项目中，随意改
hintos genre create wuxia --name 武侠   # 从零创建新题材
```
...
- `hintos update` 一键更新、`hintos init` 支持当前目录初始化
- API 错误附带中文诊断提示，`hintos doctor` 含 API 连通性测试

---

## 为什么需要 HintOS？

用 AI 写小说不是简单的"提示词 + 复制粘贴"。长篇小说很快就会崩：角色记忆混乱、物品凭空出现、同样的形容词每段都在重复、伏笔悄无声息地断掉。HintOS 把这些当工程问题来解决。
...
HintOS 提供三种交互方式，底层共享同一组原子操作：

### 1. 完整管线（一键式）

```bash
hintos write next 吞天魔帝          # 写草稿 → 审计 → 自动修订，一步到位
hintos write next 吞天魔帝 --count 5 # 连续写 5 章
```

### 2. 原子命令（可组合，适合外部 Agent 调用）

```bash
hintos draft 吞天魔帝 --context "本章重点写师徒矛盾" --json
hintos audit 吞天魔帝 31 --json
hintos revise 吞天魔帝 31 --json
```
...
```bash
hintos agent "帮我写一本都市修仙，主角是个程序员"
hintos agent "写下一章，重点写师徒矛盾"
hintos agent "先扫描市场趋势，然后根据结果创建一本新书"
```
...
配置保存在 `~/.hintos/.env`，所有项目共享。之后新建项目不用再配。

**方式二：项目级 `.env`**

```bash
hintos init my-novel     # 初始化项目
# 编辑 my-novel/.env
```
...
```bash
hintos book create --title "吞天魔帝" --genre xuanhuan  # 创建新书
hintos write next 吞天魔帝      # 写下一章（完整管线）
hintos status                   # 查看状态
hintos review list 吞天魔帝     # 审阅草稿
hintos export 吞天魔帝          # 导出全书
hintos up                       # 守护进程模式
```
...
| 命令 | 说明 |
|------|------|
| `hintos init [name]` | 初始化项目（省略 name 在当前目录初始化） |
| `hintos book create` | 创建新书（`--chapter-words` 设定字数） |
| `hintos book update [id]` | 修改书设置（`--chapter-words`、`--target-chapters`、`--status`） |
| `hintos book list` | 列出所有书籍 |
| `hintos genre list/show/copy/create` | 查看、复制、创建题材 |
| `hintos write next [id]` | 完整管线写下一章（`--words` 覆盖字数，`--count` 连写） |
| `hintos write rewrite [id] <n>` | 重写第 N 章（恢复状态快照，需确认） |
| `hintos draft [id]` | 只写草稿（`--words` 覆盖字数） |
| `hintos audit [id] [n]` | 审计指定章节 |
| `hintos revise [id] [n]` | 修订指定章节 |
| `hintos agent <instruction>` | 自然语言 Agent 模式 |
| `hintos review list [id]` | 审阅草稿 |
| `hintos review approve-all [id]` | 批量通过 |
| `hintos status [id]` | 项目状态 |
| `hintos export [id]` | 导出书籍为 txt/md |
| `hintos radar scan` | 扫描平台趋势 |
| `hintos config set-global` | 设置全局 LLM 配置（~/.hintos/.env） |
| `hintos config show-global` | 查看全局配置 |
| `hintos config set/show` | 查看/更新项目配置 |
| `hintos doctor` | 诊断配置问题（含 API 连通性测试） |
| `hintos detect [id] [n]` | AIGC 检测（`--all` 全部章节，`--stats` 统计） |
| `hintos style analyze <file>` | 分析参考文本提取文风指纹 |
| `hintos style import <file> [id]` | 导入文风指纹到指定书 |
| `hintos update` | 更新到最新版本 |
| `hintos up / down` | 启动/停止守护进程 |

`[id]` 参数在项目只有一本书时可省略，自动检测。所有命令支持 `--json` 输出结构化数据。`draft`/`write next`/`book create` 支持 `--context` 传入创作指导，`--words` 覆盖每章字数（OpenClaw 可逐章动态控制）。

## 实测数据

用 HintOS 全自动跑了一本玄幻题材的《吞天魔帝》：
...
每章自动创建状态快照。使用 `hintos write rewrite <id> <n>` 可以回滚并重新生成任意章节 — 世界状态、资源账本、伏笔钩子全部恢复到该章写入前的状态。
...
`hintos up` 启动后台循环，按计划写章。管线对非关键问题全自动运行，当审计员标记无法自动修复的问题时暂停等待人工审核。
...
原子命令 + `--json` 输出让 HintOS 可以被 OpenClaw 等 AI Agent 调用。OpenClaw 通过 `exec` 工具执行 `hintos draft`/`audit`/`revise`，读取 JSON 结果决定下一步操作。
